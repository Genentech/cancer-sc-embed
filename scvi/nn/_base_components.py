    ...
    def forward(
        ...
        *cat_list: int,
        ...
        library_size
            library size
        cnv
            (optional) tensor of shape (cell x gene) with copy number dosage. defaults to 1
        cat_list
            list of category membership(s) for this sample
        ...
        if cnv is not None:
            px_scale = px_scale * cnv
            px_scale /= px_scale.sum(axis=1).reshape(-1,1) # renormalize cells to sum to 1
